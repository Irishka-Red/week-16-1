...
paragraph.innerHTML = 'Количество найденных форм: ' + form.length;
}
...
function makeTwo() {
	...
	paragraphTwo.innerHTML = form1.getAttribute("name");
}
...
function makeThree() {
	...
	paragraphThree.innerHTML = form4.getAttribute("name");
}
...
const formAll = document.forms;
const formNames = [];

function makeFour() {
	for (let i = 0; i<formAll.length; i++){
		const fr = formAll[i];
		const formName = fr.getAttribute('name');
		formNames.push(formName);
	}
	const formNamesString = formNames.join(', ');
	practicum4.textContent = formNamesString;
	}
...
function makeFive() {
	...
	paragraphFive.innerHTML = "Количество элементов в форме: " + form3.elements.length
}
...
function makeSix() {
	...
	paragraphSix.innerHTML = "Количество элементов в форме: " + form2.elements.length
}
...
function makeNine() {
	...
	const elements = document.forms[2].elements;
	let elementsList = '';
	for (let i=0; i<elements.length; i++){
		const el = elements[i];
		const name = el.name;
...
function makeTen() {
    ...
    const elements = document.forms[3].elements;
	const meaning = elements[0].value;
	paragraphTen.textContent = meaning;
...
function makeEleven() {
	...
	const form1 = document.forms.formOne;
	const elements = document.forms.formOne.elements;
	const select = elements[2];
	let optionsValues = '';
	for (let i= 0; i<select.length; i++){
		const meaning = select[i];
...
function makeTwelve() {
	const forms = document.forms;
	const form1 = document.forms.formTwo;
	const elements = document.forms.formTwo.elements;
	const checkbox = elements[2];
	const checkboxElements = Array.from(checkbox).filter(element => element.type === 'checkbox');
	const checkboxIds = checkboxElements.map(element => element.id);
...
function checkButton(e) {
	const forms = document.forms;
	const form4 = document.forms.lastForm;
	const elements = document.lastForm.elements;
	const radio = elements[0];
	if (radio.checked){
...
formOne.addEventListener('submit', function (event) {
	...
	const elementOne = document.forms.formOne.elements[0];
	const elementTwo = document.forms.formOne.elements[1];
	const elementThree = document.forms.formOne.elements[2];
	if (elementOne.value===""||elementTwo.value===""|| elementThree.value===""){
...
const emailInput = document.forms.formOne.elements[1];
...
emailInput.oninput = function () {
	...
	if (mailFormat.test(emailValue)){
		...
		errorMessage.textContent = 'Пожалуйста, введите действительный адрес электронной почты.';
	}
};
...
const form4 = document.forms.lastForm;
const registerCheckbox = document.forms.lastForm.elements['fourthName'];
const submitButton = lastForm.querySelector('input[type="submit"]');
...
document.querySelector('.b-26').onclick = function (event) {
	...
	const form3 = document.forms.formThree;
	const element = document.forms.formThree.elements[0];
	const PlaceholderValue = element.placeholder;
	const result26Element = document.getElementById('result26');
	result26Element.textContent = PlaceholderValue;
};
...
formTwoInputs.forEach(function (input) {
	input.addEventListener('input', function () {
		...
		result27Element.textContent = "Изменение внесено";
	});
});
...
selectFormThree.addEventListener('change', function () {
	...
	result28Element.textContent = "Опция выбрана";
});
...
const nameInputThree = document.forms.formThree.elements.thirdName;
const emailInputThree = document.forms.formThree.thirdEmail;
